...
import { getNigeriaMovies } from '../../../Services/GlobalApi'; // Import the updated function
import Slider from '../../../Components/Slider';

export default function Nollywood() {
  const [nigeriaMovieList, setNigeriaMovieList] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const elementRef = useRef(null);
  const screenWidth = window.innerWidth;

  const getNigeriaMoviesList = async (page) => {
    try {
      const response = await getNigeriaMovies(page);
      console.log(response.data.results);
      setNigeriaMovieList((prevMovies) => [...prevMovies, ...response.data.results]);
    } catch (error) {
  ...
  const loadNextPage = () => {
    ...
    getNigeriaMoviesList(currentPage + 1);
  };

  useEffect(() => {
    getNigeriaMoviesList(currentPage);
  }, [currentPage]);
  ...
  return (
    ...
    <div className= "p-8 px-8 md:px-16">
      ...
      <div ref={elementRef} className='flex overflow-x-auto gap-8 scrollbar-none scroll-smooth py-5 px-3'>
        {nigeriaMovieList.map((item, index) => (
          item.poster_path && (
