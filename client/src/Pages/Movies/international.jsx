...
import { getInternationalMovies } from '../../Services/GlobalApi';
...
export default function International() {
  ...
  const fetchMovies = async (page) => {
    try {
      const response = await getInternationalMovies(page);
      console.log(response.data.results);
      setImovies((prevMovies) => [...prevMovies, ...response.data.results]);
    } catch (error) {
      console.error('Error fetching international movies:', error);
    }
  };
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (element.scrollLeft + element.clientWidth >= element.scrollWidth) {
        // Fetch more international movies when scrolled to the end
        loadNextPage();
      }
    };
  ...
  return (
    <div>
      ...
      <div ref={elementRef} className='flex overflow-x-auto gap-8 scrollbar-none scroll-smooth py-5 px-3'>
        {imovies.map((item, index) => (
          item.poster_path && <MovieCard key={index} movie={item}  />
        ))}
      </div>
      </div>
    </div>
