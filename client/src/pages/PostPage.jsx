...
export default function PostPage() {
  ...
  const [ogImageUrl, setOgImageUrl] = useState(
    "https://www.moviemaven.xyz/moviemaven.webp"
  );
    ...
    const fetchPost = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setOgImageUrl(
            data.posts[0].image || "https://www.moviemaven.xyz/moviemaven.webp"
          );
          setLoading(false);
          setError(false);
        }
    ...
    try {
      const fetchRelatedPosts = async () => {
        const res = await fetch(
          `/api/post/getposts?category=${post && post.category}&limit=3`
        );
        const data = await res.json();
        if (res.ok) {
